    ...
    private UVCCameraHelper.OnMyDevConnectListener listener = new UVCCameraHelper.OnMyDevConnectListener() {
        ...
        public void onConnectDev(UsbDevice device, boolean isConnected) {
            if (!isConnected) {
                ...
                mMatrix = new Matrix(mTextureView.getMatrix());
                // initialize seekbar
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(mCameraHelper != null && mCameraHelper.isCameraOpened()) {
                            ...
                            mSeekBrightness.setProgress(brightProcess);
                            mSeekContrast.setProgress(contrastProcess);
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    mTvBrightness.setText("" + brightProcess + "%");
                                    mTvContrast.setText("" +  contrastProcess + "%");

                                    mCenterX = (mTextureView.getRight() - mTextureView.getLeft()) / 2;
                                    mCenterY = (mTextureView.getBottom() - mTextureView.getTop()) / 2;
                                    mMatrix.setScale(5, 5, mCenterX, mCenterY);
                                    mTextureView.setTransform(mMatrix);
                                    mTextureView.postInvalidate();
                                    mTvScale.setText("x 1");
                                }
                            });
        ...
        mSeekScale.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                if(mCameraHelper != null && mCameraHelper.isCameraOpened() && mMatrix != null) {
                    mCenterX = (mTextureView.getRight() - mTextureView.getLeft()) / 2;
                    mCenterY = (mTextureView.getBottom() - mTextureView.getTop()) / 2;
                    mMatrix.setScale(progress == 0 ? 1 : progress, progress == 0 ? 1 : progress, mCenterX, mCenterY);
                    mTextureView.setTransform(mMatrix);
                    mTextureView.postInvalidate();
                    mTvScale.setText("x " + (progress == 5 ? 1 : progress - 5));
                ...
                if (size != null) {
                    if (size.fps.length >= 1){
                        for (int i = 0; i < size.fps.length; i ++){
                            String key = size.frame_type == 4 ? "YUY2 " : "MJPG " + size.width + "x" + size.height + "  " + ((int)size.fps[i]) + "fps";
                            resolutions.add(key);
                            selecs.put(key, size);
                        }
                    }
        ...
        if (list != null && list.size() != 0) {
            ...
            for (Size size : list) {
                if (size != null) {
                    if (size.fps.length >= 1){
                        for (int i = 0; i < size.fps.length; i ++){
                            resolutions.add(size.frame_type == 4 ? "YUY2 " : "MJPG " + size.width + "x" + size.height + "  " + ((int)size.fps[i]) + "fps");
                        }
                    }
                }
            }
