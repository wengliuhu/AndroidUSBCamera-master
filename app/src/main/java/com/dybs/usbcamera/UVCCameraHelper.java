...
//                }).start();
    ...
    public void updateFormat(int format) {
        ...
        if (mCameraHandler != null && mCameraHandler.getParams() != null) {
    ...
    public void updateFormat(int width, int height, int format) {
        AbstractUVCCameraHandler.UVCParam param = null;
        if (mCameraHandler != null && mCameraHandler.getParams() != null) {
            param = mCameraHandler.getParams();
        }else param = new AbstractUVCCameraHandler.UVCParam();
        param.format = format;
        updateResolution(width, height, param);
    }

    public void updateResolution(int width, int height, AbstractUVCCameraHandler.UVCParam param) {
        if (previewWidth == width && previewHeight == height && param == null) {
            return;
        }
...
//            throw new NullPointerException("mUSBMonitor ="+mUSBMonitor+"deviceFilters=;"+deviceFilters);
    ...
    public List<Size> getSupportedPreviewSizes(int formate) {
        if (mCameraHandler == null)
            return null;
        return mCameraHandler.getSupportedPreviewSizes(formate);
    }
